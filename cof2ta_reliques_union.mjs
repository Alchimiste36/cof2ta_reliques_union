Hooks.once("init", () => {
  console.info("COF2TA Les Reliques de l'Union | Initialisation du module...")
  ...
  console.info("COF2TA Les Reliques de l'Union | Fin de l'initialisation du module")
})

Hooks.once("ready", async () => {
  console.info("COF2TA Les Reliques de l'Union | Module prêt")
})
...
Hooks.on("renderJournalEntrySheet", (application, element, context, options) => {
  if (application.document.getFlag("cof2ta_reliques_union", "isJournalCOF") === true) {
    element.classList.add("journal-cof2ta_reliques_union")
  }
})
...
Hooks.on("renderCOSidebarMenu", async (application, html, context, options) => {
  ...
  if (element) {
    const renderedHtml = await foundry.applications.handlebars.renderTemplate("modules/cof2ta_reliques_union/templates/sidebar-menu.hbs", {
      user: game.user,
    })
